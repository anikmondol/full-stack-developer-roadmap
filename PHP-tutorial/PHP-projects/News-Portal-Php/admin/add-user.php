...
if (isset($_REQUEST["save"])) {
    ...
    $fname = mysqli_real_escape_string($conn, trim($_REQUEST["fname"]));
    $lname = mysqli_real_escape_string($conn, trim($_REQUEST["lname"]));
    $user = mysqli_real_escape_string($conn, trim($_REQUEST["user"]));
    $password = mysqli_real_escape_string($conn, sha1(trim($_REQUEST["password"])));
    $role = mysqli_real_escape_string($conn, trim($_REQUEST["role"]));

    $check_user_query = "SELECT username FROM `user` WHERE username = '$user'";
